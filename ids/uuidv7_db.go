...
func (g UUIDv7DBGenerator) Generate() string {
	// UUIDv7 is generated by the database, so this might return an empty string or a placeholder.
	return ""
}
...
func (g UUIDv7DBGenerator) CreateTable(ctx context.Context, pool *pgxpool.Pool) error {
	...
	_, err = pool.Exec(ctx, "CREATE TABLE IF NOT EXISTS uuidv7_table (id UUID PRIMARY KEY DEFAULT uuid7(), n BIGINT NOT NULL)")
	return err
}

func (g UUIDv7DBGenerator) DropTable(ctx context.Context, pool *pgxpool.Pool) error {
	_, err := pool.Exec(ctx, "DROP TABLE IF EXISTS uuidv7_table")
	return err
}

func (g UUIDv7DBGenerator) InsertRecord(ctx context.Context, pool *pgxpool.Pool) error {
	_, err := pool.Exec(ctx, "INSERT INTO uuidv7_table (n) VALUES (1)")
	return err
}

func (g UUIDv7DBGenerator) BulkWriteRecords(ctx context.Context, pool *pgxpool.Pool, count uint64) error {
	_, err := pool.Exec(ctx, "INSERT INTO uuidv7_table (n) SELECT g.n FROM generate_series(1, $1) AS g(n)", count)
	return err
}

func (g UUIDv7DBGenerator) CollectStats(ctx context.Context, pool *pgxpool.Pool) (map[string]any, error) {
...
func (g UUIDv7DBGenerator) LoadUUID7Function(ctx context.Context, pool *pgxpool.Pool) error {
	sql := `
